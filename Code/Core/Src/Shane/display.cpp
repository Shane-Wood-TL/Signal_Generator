...
void display::initDisplay() {
	...
	//draw shift, amp to the screen for ch1 and ch2
	drawWordsNorm(0);
	writeBuffer();
	drawWordsNorm(1);
	return;
...
void display::sendData(const uint8_t *data,const uint16_t len) {
...
//set up the display
display::display(SPI_HandleTypeDef *hspi1I, displayQueue *displayQueueI,const  ssd1306Setup *setup){
...
//write number values to the display
void display::writeValues(const uint8_t page, const bool echo){
...
void display::drawWordsNorm(const uint8_t row) {
	if(!((row == 0) || (row==1))){
		return;
	}
	assert((row == 0) || (row==1));
	...
	if(row==1){
		//row 1 is the only one that is replaceable past first run, top 2 words are not replaced
...
void display::writeSection(const uint8_t page,const uint8_t columnStart, const uint8_t columnStop){
	if(page > 7){
		return;
...
void display::displaySignalType(const WaveShape shape, const uint8_t Channel) {
	...
	if (shape == SINE) {
		...
		//wave not valid, display nothing
		return;
	}
	assert(symbolToWrite != nullptr);
...
void display::getNewValues() {
	...
	assert((currentState >=0) && (currentState < 5));
	switch(currentState){
	case(0):{
